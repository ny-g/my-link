...
export function Header() {
  ...
  const handleCopyLink = (e: React.MouseEvent) => {
    e.preventDefault(); // Prevent closing the menu immediately if possible, or just let it close
    if (userProfile?.slug) {
      const url = `${window.location.origin}/${userProfile.slug}`;
                  ...
                  <Menu.Portal>
                    <Menu.Positioner align="end" sideOffset={8}>
                      <Menu.Popup className="z-[9999] w-64 rounded-2xl bg-white dark:bg-zinc-900 p-2 shadow-xl ring-1 ring-slate-200 dark:ring-zinc-800 outline-none transform origin-top-right transition-all data-[starting-style]:scale-95 data-[starting-style]:opacity-0 data-[ending-style]:scale-95 data-[ending-style]:opacity-0">
                        ...
                        {/* Menu Items */}
                        <Menu.Item className="flex items-center gap-2 w-full px-3 py-2.5 text-sm font-medium rounded-xl text-slate-700 dark:text-zinc-300 hover:bg-slate-100 dark:hover:bg-zinc-800 outline-none cursor-pointer select-none transition-colors">
